...
function generateAppEntry(model: SemanticAppModel): GeneratedFile {
    ...
    const hasSupabase = (model.backendIntegrations ?? []).some(b => b.kind === 'supabase');
    ...
    if (hasSupabase) {
        lines.push('                .task {');
        lines.push('                    await SupabaseManager.shared.restoreSession()');
        lines.push('                }');
    }
    lines.push('        }');
    if (hasSwiftData) {
...
function generateAuthManager(model: SemanticAppModel): string {
    const hasSupabaseAuth = model.auth?.provider === 'supabase' ||
        (model.backendIntegrations ?? []).some(b => b.kind === 'supabase' && b.features.includes('auth'));

    if (hasSupabaseAuth) {
        return generateSupabaseAuthManager();
    }

    // Generic APIClient-based auth
    const entities = model.entities ?? [];
...
function generateSupabaseAuthManager(): string {
    return `// Generated by Morphkit — Supabase Auth integration
// Uses SupabaseManager for authentication instead of APIClient
...
final class AuthManager {
    ...
    var isAuthenticated: Bool { SupabaseManager.shared.isAuthenticated }
    var currentUserId: UUID? { SupabaseManager.shared.currentUserId }
    ...
    func register(name: String? = nil, email: String, password: String) async throws {
        try await SupabaseManager.shared.signUp(email: email, password: password)
    }

    func signInWithMagicLink(email: String) async throws {
        try await SupabaseManager.shared.signInWithMagicLink(email: email)
    }
...
/**
 * Generate a Color+Theme.swift extension with extracted app colors.
 * Returns null if the theme only has default colors (nothing extracted).
 */
function generateColorTheme(model: SemanticAppModel): GeneratedFile | null {
    const theme = model.theme;
    if (!theme) return null;

    const colors = theme.colors;
    // Check if any colors differ from defaults (indicating they were extracted)
    const defaults = {
        primary: '#007AFF', secondary: '#5856D6', accent: '#FF9500',
        background: '#FFFFFF', surface: '#F2F2F7', error: '#FF3B30',
        success: '#34C759', warning: '#FF9F0A',
    };

    const hasCustomColors = Object.entries(colors).some(([key, val]) =>
        val && defaults[key as keyof typeof defaults] !== val
    );

    if (!hasCustomColors) return null;

    const lines: string[] = [];
    lines.push('// Generated by Morphkit — App color palette extracted from web app theme');
    lines.push('');
    lines.push('import SwiftUI');
    lines.push('');
    ...
    const colorEntries: Array<[string, string]> = [
        ...
        ['appWarning', colors.warning],
    ];

    for (const [name, hex] of colorEntries) {
        if (hex) {
            lines.push(`    static let ${name} = Color(hex: "${hex}")`);
        }
    }

    lines.push('}');
    lines.push('');
    lines.push('// MARK: - Hex Color Initializer');
    lines.push('');
    lines.push('extension Color {');
    ...
    return {
        path: 'Theme/Color+Theme.swift',
        content: lines.join('\n'),
        sourceMapping: 'morphkit:theme',
        confidence: 'high',
        warnings: [],
    };
}
...
/**
 * Validate generated Swift files using two passes:
 * 1. Per-file `swiftc --parse` for syntax errors (fast, catches most issues)
 * 2. Full `swift build` for cross-file type errors (slower, catches all issues)
 * Returns an array of warning strings for any errors found.
...
async function validateSwiftSyntax(appDir: string, files: GeneratedFile[]): Promise<string[]> {
    // Allow tests to skip expensive swift validation
    if (process.env.MORPHKIT_SKIP_SWIFT_VALIDATION === '1') return [];

    const warnings: string[] = [];

    // Check if swift/swiftc is available on PATH
    try {
    ...
    // Pass 1: Per-file syntax check (fast)
    ...
    // Pass 2: Full project build (catches cross-file type errors)
    // The Package.swift is at the parent of appDir
    ...
    try {
        ...
        if (output.includes('error:')) {
            ...
            for (const line of lines) {
                const match = line.match(/([^/]+\.swift):(\d+):\d+:\s*error:\s*(.+)$/);
                if (match) {
                    const [, fileName, lineNum, message] = match;
                    // Only add if not already reported by per-file check
                    const warning = `[${fileName}:${lineNum}] ${message}`;
                    if (!warnings.some(w => w.includes(message))) {
...
export async function generateXcodeProject(
    ...
    // 5b. Color+Theme.swift (app-wide color palette from Tailwind/theme config)
    const themeFile = generateColorTheme(model);
    if (themeFile) allFiles.push(themeFile);
    ...
    // --- Generate placeholder models for entity types referenced in views but not in models ---
    // This catches cases where the analyzer didn't extract entities (e.g., component-local interfaces)
    // but the view generator inferred entity names from screen names.
    const modelTypeNames = new Set<string>();
    // Scan ALL generated swift files for type declarations
    for (const file of allFiles) {
        if (!file.path.endsWith('.swift')) continue;
        const typeMatches = file.content.matchAll(/^(?:struct|class|enum)\s+(\w+)/gm);
        for (const m of typeMatches) {
            modelTypeNames.add(m[1]);
        }
    }
    // Scan view files for @State property types that reference missing entity types
    const missingTypes = new Set<string>();
    // Swift/SwiftUI standard library types and Foundation types that should never be generated as placeholders
    const swiftBuiltins = new Set([
        'String', 'Int', 'Double', 'Float', 'Bool', 'Date', 'UUID', 'Data', 'URL',
        'Any', 'Void', 'Never', 'Color', 'Image', 'View', 'Text', 'Button',
        'NavigationStack', 'NavigationPath', 'TabView', 'List', 'Form', 'Section',
        'SortOrder', 'Locale', 'TimeZone', 'Calendar', 'IndexSet', 'Range',
        'Error', 'Optional', 'Array', 'Dictionary', 'Set', 'Result',
        'CGFloat', 'CGPoint', 'CGSize', 'CGRect',
    ]);
    for (const file of allFiles) {
        if (!file.path.startsWith('Views/') && !file.path.startsWith('State/')) continue;
        // Match @State type declarations like: @State private var foo: TypeName? or [TypeName]
        const stateTypeRe = /(?:@State\s+(?:private\s+)?var\s+\w+:\s*\[?)(\b[A-Z][A-Za-z0-9]+\b)/g;
        let match: RegExpExecArray | null;
        while ((match = stateTypeRe.exec(file.content)) !== null) {
            const typeName = match[1];
            if (!swiftBuiltins.has(typeName) && !modelTypeNames.has(typeName)) {
                missingTypes.add(typeName);
            }
        }
    }
    // Generate minimal placeholder structs for missing types
    for (const typeName of missingTypes) {
        const placeholderContent = `// Generated by Morphkit — placeholder model
// This entity was inferred from screen structure but not found in source types.
// Replace with your actual data model.

import Foundation

struct ${typeName}: Codable, Identifiable, Hashable {
    let id: String
    var name: String
...
`;
        allFiles.push({
            path: `Models/${typeName}.swift`,
            content: placeholderContent,
            sourceMapping: 'morphkit:placeholder',
            confidence: 'low' as const,
            warnings: [`${typeName} is a placeholder — replace with your actual model`],
        });
        modelTypeNames.add(typeName);
    }
