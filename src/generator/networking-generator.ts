...
function generateAPIConfiguration(model: SemanticAppModel): string {
    // Priority: model.apiBaseURL (from env scanning) > inferred from endpoints
    const baseURL = model.apiBaseURL ?? inferBaseURL(model.apiEndpoints ?? []);
...
export function generateNetworkingLayer(model: SemanticAppModel): GeneratedFile[] {
    ...
    if (supabaseIntegration) {
        files.push({
            ...
            content: generateSupabaseManager(supabaseIntegration.features, supabaseIntegration.configEnvVars),
            sourceMapping: 'morphkit:supabase',
            confidence: 'high',
            warnings: supabaseIntegration.configEnvVars.length > 0 ? [] : ['Update SUPABASE_URL and SUPABASE_ANON_KEY in SupabaseManager'],
        });
    }
    ...
    if (stripeIntegration) {
        files.push({
            ...
            content: generatePaymentManager(model),
...
function generateSupabaseManager(features: string[], configEnvVars: string[] = []): string {
    // Extract detected Supabase credentials from env vars
    const envMap = new Map(configEnvVars.map(v => {
        const [k, ...rest] = v.split('=');
        return [k, rest.join('=')] as [string, string];
    }));
    const detectedURL = envMap.get('NEXT_PUBLIC_SUPABASE_URL') ?? envMap.get('SUPABASE_URL');
    const detectedKey = envMap.get('NEXT_PUBLIC_SUPABASE_ANON_KEY') ?? envMap.get('SUPABASE_ANON_KEY');
    ...
    if (detectedURL && detectedKey) {
        // Credentials detected from env — wire them directly
        lines.push(`    private static let supabaseURL = URL(string: "${detectedURL}")!`);
        lines.push(`    private static let supabaseAnonKey = "${detectedKey}"`);
    } else {
        // No credentials found — use Info.plist pattern for configuration
        lines.push('    // Configure in Info.plist or Xcode build settings:');
        lines.push('    // SUPABASE_URL = https://your-project.supabase.co');
        lines.push('    // SUPABASE_ANON_KEY = your-anon-key');
        lines.push('    private static let supabaseURL = URL(string: Bundle.main.infoDictionary?["SUPABASE_URL"] as? String ?? "https://YOUR_PROJECT.supabase.co")!');
        lines.push('    private static let supabaseAnonKey = (Bundle.main.infoDictionary?["SUPABASE_ANON_KEY"] as? String) ?? "YOUR_ANON_KEY"');
    }
    lines.push('');
    ...
    if (features.includes('auth')) {
        ...
        lines.push('    }');
        lines.push('');
        lines.push('    func resetPassword(email: String) async throws {');
        lines.push('        try await client.auth.resetPasswordForEmail(email)');
        lines.push('    }');
        lines.push('');
        lines.push('    func updatePassword(newPassword: String) async throws {');
        lines.push('        try await client.auth.update(user: UserAttributes(password: newPassword))');
        lines.push('    }');
        lines.push('');
        lines.push('    func verifyOTP(email: String, token: String) async throws {');
        lines.push('        let session = try await client.auth.verifyOTP(email: email, token: token, type: .email)');
        lines.push('        currentUserId = session.user.id');
        lines.push('        isAuthenticated = true');
        lines.push('    }');
        lines.push('');
...
function generatePaymentManager(model: SemanticAppModel): string {
    // Find Stripe-related API endpoints
    const endpoints = model.apiEndpoints ?? [];
    const checkoutEp = endpoints.find(ep =>
        (ep.method === 'POST') && (
            ep.url.includes('checkout') || ep.url.includes('topup') ||
            ep.url.includes('billing') || ep.url.includes('payment')
        )
    );
    const checkoutPath = checkoutEp
        ? cleanURL(checkoutEp.url)?.replace(/^https?:\/\/[^/]+/, '') ?? '/api/billing/topup'
        : '/api/billing/topup';
...
final class PaymentManager {
    ...
    func startCheckout(amount: Double, description: String) async throws {
        ...
        var request = URLRequest(url: APIConfiguration.baseURL.appending(path: "${checkoutPath}"))
