...
const JUNK_TYPE_NAMES = new Set([
    ...
    'promise<any>', 'Promise<any>', 'void', 'undefined', 'null',
]);
...
function deduplicateEndpoints(endpoints: ApiEndpoint[]): ApiEndpoint[] {
    ...
    for (const ep of endpoints) {
        ...
        const key = `${(ep.method ?? 'GET').toUpperCase()}:${cleaned}`;
...
function generateEndpointMethod(endpoint: ApiEndpoint, entities: Entity[] = []): string {
    ...
    // Add a TODO comment when the return type couldn't be resolved
    if (returnType === 'Any' || returnType === '[Any]') {
        lines.push(`    // TODO: Replace \`${returnType}\` with the actual response type`);
    }

    const hasBody = method !== 'GET' && endpoint.requestBody != null;

    if (returnType && returnType !== 'Void') {
        lines.push(`    func ${functionName}(${params.join(', ')}) async throws -> ${returnType} {`);
    } else if (method === 'DELETE' || (method === 'POST' && !returnType)) {
        lines.push(`    func ${functionName}(${params.join(', ')}) async throws {`);
    } else {
        lines.push(`    func ${functionName}(${params.join(', ')}) async throws -> ${returnType ?? 'Void'} {`);
    }
...
function generateFunctionName(endpoint: ApiEndpoint): string {
    ...
    const segments = cleaned
        .split('/')
        .filter((s) => s && !s.startsWith(':') && !s.startsWith('{') && s !== 'api');
