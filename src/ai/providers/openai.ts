 ...
 * Supports any OpenAI-compatible endpoint via baseURL config,
 * making it usable with OpenAI, Azure OpenAI, local models, etc.
 */
...
function extractJSON(text: string): string {
  ...
  throw new Error(`Failed to extract JSON from OpenAI response: ${trimmed.slice(0, 200)}...`);
}
      ...
      try {
        const response = await Promise.race([
          ...
          new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error('OpenAI API call timed out')), TIMEOUT_MS),
          ),
        ]);
